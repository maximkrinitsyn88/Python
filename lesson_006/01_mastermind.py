...
from mastermind_engine import guess_the_number, checking_the_entered_number, check_win
from termcolor import cprint, colored

cprint(colored('Вам загадано четырехзначное число', color='red'))
hidden_number = guess_the_number()
game_duration = 0
while True:
    number = checking_the_entered_number()
    if number:
        game_duration += 1
    if check_win(number=number, hidden_number=hidden_number):
        print('Количество ходов: ', game_duration)
        offer_to_repeat = input(colored('Хотите еще партию? (Ответьте `Да`, чтобы принять): ', color='green'))
        if offer_to_repeat == 'Да':
            cprint(colored('Вам загадано четырехзначное число', color='red'))
            game_duration = 0
            hidden_number = guess_the_number()
        else:
